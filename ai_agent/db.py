...
def init_ai_agent_db():
    ...
    try:
        with conn.cursor() as c:
            c.execute('''CREATE TABLE IF NOT EXISTS ai_interview_bookings (
                id SERIAL PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
                booking_date DATE NOT NULL,slot_number SMALLINT NOT NULL CHECK(slot_number BETWEEN 1 AND 3),
                questions_used SMALLINT NOT NULL DEFAULT 0,status VARCHAR(20) NOT NULL DEFAULT 'booked',
            ...
            # Locks from a killed worker / old deployment must never block a user forever.
            c.execute("UPDATE ai_interview_bookings SET request_in_flight=FALSE,request_started_at=NULL WHERE request_in_flight=TRUE AND (request_started_at IS NULL OR request_started_at < CURRENT_TIMESTAMP - INTERVAL '60 seconds')")
            ...
            END $$''')
            c.execute('''CREATE TABLE IF NOT EXISTS ai_interview_messages (
                id SERIAL PRIMARY KEY,booking_id INTEGER NOT NULL REFERENCES ai_interview_bookings(id) ON DELETE CASCADE,
                role VARCHAR(20) NOT NULL CHECK(role IN ('user','assistant','review')),content TEXT NOT NULL,
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)''')
            c.execute("ALTER TABLE ai_interview_messages ADD COLUMN IF NOT EXISTS agent VARCHAR(20) NOT NULL DEFAULT 'business'")
            c.execute('''CREATE TABLE IF NOT EXISTS ai_interview_specs (
                id SERIAL PRIMARY KEY,booking_id INTEGER NOT NULL REFERENCES ai_interview_bookings(id) ON DELETE CASCADE,
                mermaid TEXT NOT NULL DEFAULT '',api_spec TEXT NOT NULL DEFAULT '',business_rules TEXT NOT NULL DEFAULT '',
                created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)''')
...
def _booking(r):
    return None if not r else {'id':r[0],'user_id':r[1],'booking_date':r[2],'slot_number':r[3],'questions_used':r[4],'status':r[5]}

def get_bookings(start_date,end_date):
    conn=get_db_connection()
    try:
        with conn.cursor() as c:
            c.execute('''SELECT b.id,b.user_id,a.username,b.booking_date,b.slot_number,b.questions_used,b.status FROM ai_interview_bookings b JOIN accounts a ON a.id=b.user_id WHERE b.booking_date BETWEEN %s AND %s AND b.status IN ('booked','started','finished') ORDER BY b.booking_date,b.slot_number''',(start_date,end_date))
...
def get_booking_for_user_on_date(user_id,booking_date):
    ...
    try:
        with conn.cursor() as c:
            c.execute('''SELECT b.id,b.user_id,b.booking_date,b.slot_number,COUNT(m.id) FILTER(WHERE m.role='user')::int,b.status FROM ai_interview_bookings b LEFT JOIN ai_interview_messages m ON m.booking_id=b.id WHERE b.user_id=%s AND b.booking_date=%s AND b.status IN ('booked','started','finished') GROUP BY b.id,b.user_id,b.booking_date,b.slot_number,b.status LIMIT 1''',(user_id,booking_date))
...
def get_next_booking_for_user(user_id,today=None):
    ...
    try:
        with conn.cursor() as c:
            c.execute("SELECT id,user_id,booking_date,slot_number,questions_used,status FROM ai_interview_bookings WHERE user_id=%s AND booking_date>=%s AND status IN ('booked','started') ORDER BY booking_date LIMIT 1",(user_id,today));return _booking(c.fetchone())
    finally: conn.close()
...
def cancel_booking(user_id,booking_id):
    ...
    try:
        with conn.cursor() as c:
            c.execute("UPDATE ai_interview_bookings SET status='cancelled',request_in_flight=FALSE,request_started_at=NULL WHERE id=%s AND user_id=%s AND status='booked' AND booking_date>=CURRENT_DATE RETURNING id",(booking_id,user_id));row=c.fetchone()
...
def get_latest_spec(booking_id):
    ...
    try:
        with conn.cursor() as c:
            c.execute('SELECT mermaid,api_spec,business_rules FROM ai_interview_specs WHERE booking_id=%s ORDER BY id DESC LIMIT 1',(booking_id,));r=c.fetchone();return {'mermaid':r[0],'api_spec':r[1],'business_rules':r[2]} if r else {'mermaid':'','api_spec':'','business_rules':''}
    finally:conn.close()
...
def begin_question(user_id,booking_id):
    ...
    try:
        with conn.cursor() as c:
            c.execute('''UPDATE ai_interview_bookings
                SET request_in_flight=TRUE,request_started_at=CURRENT_TIMESTAMP
                WHERE id=%s AND user_id=%s AND status IN ('booked','started')
                  AND (request_in_flight=FALSE OR request_started_at IS NULL OR request_started_at < CURRENT_TIMESTAMP - INTERVAL '60 seconds')
                RETURNING id''',(booking_id,user_id));row=c.fetchone()
        conn.commit()
...
def release_question(user_id,booking_id):
    conn=get_db_connection()
    try:
        with conn.cursor() as c:c.execute('UPDATE ai_interview_bookings SET request_in_flight=FALSE,request_started_at=NULL WHERE id=%s AND user_id=%s',(booking_id,user_id))
        conn.commit()
    except Exception:conn.rollback();raise
    finally:conn.close()
...
def save_exchange(user_id,booking_id,agent,question,answer):
    ...
    try:
        with conn.cursor() as c:
            c.execute('SELECT status,request_in_flight FROM ai_interview_bookings WHERE id=%s AND user_id=%s FOR UPDATE',(booking_id,user_id));r=c.fetchone()
            if not r or r[0]=='finished':raise ValueError('Симуляция недоступна')
            if not r[1]:raise ValueError('Запрос уже завершён или отменён')
...
def get_review(booking_id):
    conn=get_db_connection()
    try:
        with conn.cursor() as c:c.execute("SELECT content FROM ai_interview_messages WHERE booking_id=%s AND role='review' ORDER BY id DESC LIMIT 1",(booking_id,));r=c.fetchone();return r[0] if r else None
    finally:conn.close()
