...
def cancel_booking(user_id,booking_id):
    ...
    try:
        with conn.cursor() as c:
            c.execute("UPDATE ai_interview_bookings SET status='cancelled' WHERE id=%s AND user_id=%s AND status='booked' AND booking_date>=CURRENT_DATE RETURNING id",(booking_id,user_id)); row=c.fetchone()
...
def release_question(user_id,booking_id):
    conn=get_db_connection()
    try:
        with conn.cursor() as c: c.execute('UPDATE ai_interview_bookings SET request_in_flight=FALSE WHERE id=%s AND user_id=%s',(booking_id,user_id))
        conn.commit()
    except Exception: conn.rollback(); raise
    finally: conn.close()
...
def save_exchange(user_id,booking_id,agent,question,answer):
    ...
    try:
        with conn.cursor() as c:
            ...
            c.execute("UPDATE ai_interview_bookings SET questions_used=%s,status='started',request_in_flight=FALSE WHERE id=%s",(used+1,booking_id))
        conn.commit()
    except Exception: conn.rollback(); raise
    finally: conn.close()
